...
class JavaRMIAdapterTest {
    ...
    @Test
    void remote() {
        ConnectionAdapter remote = localAdapter.remote(localAddress + ":" + REMOTE_PORT);
    ...
    void getAddress() {
        ...
        Assertions.assertEquals(localAddress + ":" + LOCAL_PORT, localAdapter.getAddress());
        Assertions.assertEquals(localAddress + ":" + REMOTE_PORT, remoteAdapter.getAddress());
        // Assert that the method returns the correct value when requested from a remote server.
        ConnectionAdapter remote = localAdapter.remote(localAddress + ":" + REMOTE_PORT);
        Assertions.assertNotNull(remote);
        try {
            Assertions.assertEquals(localAddress + ":" + REMOTE_PORT, remote.getAddress());
        } catch (RemoteException e) {
            Assertions.fail(e);
        }
    }
